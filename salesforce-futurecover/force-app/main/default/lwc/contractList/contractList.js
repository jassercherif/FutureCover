...
import getContracts from '@salesforce/apex/ObjectsController.getContracts';
...
const columns = [
    { label: 'Contract Number', fieldName: 'ContractNumber', type: 'text' },
    {
        ...
        type: 'text',
        cellAttributes: {
            class: { fieldName: 'statusClass' } // optionnel pour la couleur par statut
        }
    },
...
wiredContracts(result) {
    ...
    deleteOpportunity(recordId) {
        if (confirm('Are you sure you want to delete this contract?')) {
            deleteRecord(recordId)
                .then(() => {
                    this.showToast('Success', 'Contract deleted', 'success');
    ...
    async downloadCSV() {
        ...
        hiddenElement.download = 'ContractList.csv';
